...
\author{
  S{\o}ren H{\o}jsgaard\\Department of Genetics and Biotechnology \\Aarhus University, Denmark
}
...
as probabilistic networks and Bayesian networks. More
information about the package might be available from the webpage 
\url{http://gbi.agrsci.dk/~shd/}. 
...
\begin{figure}[ht]
  \centering
  \includegraphics[height=5cm]{fig/chest-LS2}
  \caption{Chest clinic example from LS.}
  \label{fig:chest-LS}
\end{figure}


\subsection{Building a \grain}
...
\subsection{Queries to \grain s}
\label{sec:xxx}

Suppose we are given the finding (evidence) that a set of variables $E\subset V$
...
With this  finding, we are often interested in the
...
of seeing a specific finding, i.e.\ $p(E=e^*)$.
...
\item Create the \grain\ from the conditional probability tables:
...
\item The \grain\ can be queried to give marginal probabilities:
...
\item Findings can be entered as:
...
\item The \grain\ can be queried again:
