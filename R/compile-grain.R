                          ...
                          control=object$control, trace=0,...)
{
  ...
  if (inherits(object, "cpt-grain")){
    ans <- .compileCPT(object, method=method, propagate=propagate, root=root, smooth=smooth, control=control, trace=trace)

  } else {
    if (inherits(object, "list-grain")){

      ans <- .compileLIST(object, method=method, propagate=propagate, root=root, smooth=smooth, control=control, trace=trace)
    }
    else {
      if (inherits(object, "dag-grain")){
        ...
        ## UG
        ans <- .compileUG(object, method=method, propagate=propagate, root=root, smooth=smooth, control=control, trace=trace)      
      }
    }  
  }
  ...
  if (length(r$cliques)>1){
    ...
    if (nrow(ft)==0){
      ans$jt <- new("graphNEL", nodes = as.character(V), edgemode = "directed")
    } else {
      ans$jt <- ftM2graphNEL(ft, V=as.character(V), edgemode="directed")
    }
  } else {
    ans$jt <- new("graphNEL", nodes = "1", edgemode = "directed")
  }
